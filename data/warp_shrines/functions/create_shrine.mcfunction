...
execute align xyz positioned ~0.5 ~ ~0.5 run summon armor_stand ~ ~ ~ {Tags:["ws_shrine","ws_unlinked"],Invisible:true,CustomNameVisible:true,CustomName:'"Rename Me to Warp Location"'} 

# Update scoreboard variables
scoreboard players add $ws_shrineCount ws_shrineCount 1

#execute as @e[tag=ws_shrine,limit=1,sort=nearest] run scoreboard players add $ws_shrineCount ws_shrineCount 1
