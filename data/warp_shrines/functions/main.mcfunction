...
execute as @e[type=item,nbt={Item:{tag:{tag:ws_stone}}}] at @s if block ~ ~-1 ~ minecraft:beacon unless entity @e[tag=ws_shrine,distance=..1] run function warp_shrines:shrines/create_shrine

# Set link between shrines
execute as @e[type=item,nbt={Item:{tag:{tag:ws_pearl}}}] at @s if entity @e[distance=..1,tag=ws_unlinked] run function warp_shrines:shrines/thrown_pearl
