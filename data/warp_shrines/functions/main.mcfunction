...
execute as @e[type=item,nbt={Item:{id:"minecraft:nether_star",Count:1b,tag:{display:{Name:'[{"text":"Warp Stone","italic":true,"color":"dark_aqua"}]'}}}}] at @s if block ~ ~-1 ~ minecraft:beacon if entity @e[tag=!ws_shrine] run function warp_shrines:shrines/create_shrine
